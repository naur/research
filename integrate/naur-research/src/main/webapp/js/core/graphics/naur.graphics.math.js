...
define(['jquery', 'naur', 'naur.math', 'naur.graphics.js'], function ($, NAUR) {
    NAUR.Graphics.Math = (function () {
            ...
            compile:function (n) {
                ...
                if (funcs.length) {
                    for (var i = 0; i < funcs.length; i++) {
                        ...
                        builder += "ctx.beginPath();var x=coordinate.X1;ctx.move(x," + jsc + ");";
                        if (0 && singularities.length) {
                            ...
                            builder += "for(var x=coordinate.X1;x<coordinate.X2;x+=(coordinate.X2-coordinate.X1)/coordinate.width){" + "ctx.line(x," + jsc + ");}ctx.stroke();";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "ctx.line(coordinate.X2+2,coordinate.Y1+2);ctx.line(coordinate.X1-2,coordinate.Y1+2);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "ctx.line(coordinate.X2+2,coordinate.Y2-2);ctx.line(coordinate.X1-2,coordinate.Y2-2);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            }
                ...
                if (yfuncs.length) {
                    for (var i = 0; i < yfuncs.length; i++) {
                        ...
                        //begin the path. Colour (fill and stroke) is already done for us in ui.js
                        builder += "ctx.beginPath();var y=coordinate.Y2;ctx.move(" + jsc + ",y);";
                        if (0 && singularities.length) {
                            //Better plot logic.
                            //For example, this would allow us to plot y=sqrt(1-x^2) from [-1,1] exactly with no abrupt endings.
                            ...
                            //The loop is within the function because calling a function is too slow in javascript (last time I checked)
                            //Plot each point: (jsc is the expression generated by CAS)
                            builder += "for(var y=coordinate.Y2;y<coordinate.Y1;y+=(coordinate.Y1-coordinate.Y2)/coordinate.height){" + "ctx.line(" + jsc + ",y);}";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "ctx.line(coordinate.X2+2,coordinate.Y1+2);ctx.line(coordinate.X2+2,coordinate.Y2-2);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "ctx.line(coordinate.X1-2,coordinate.Y1+2);ctx.line(coordinate.X1-2,coordinate.Y2-2);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            }
                            builder += "ctx.stroke();";
                    ...
                    for (var i = 0; i < rfuncs.length; i++) {
                        ...
                        if (0 && singularities.length) {
                            ...
                            /*
                             ...
                             asin(y/r)=f(atan(y/x))
                             */
                            builder += "var rinc=(rmax-rmin)/coordinate.width;for(var r=rmin;r<rmax;r+=rinc){var thtmp=(" + jsc + ");ctx.line(r*cos(thtmp),r*sin(thtmp));}";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "ctx.line(coordinate.X2+2,coordinate.Y1+2);ctx.line(coordinate.X2+2,coordinate.Y2-2);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "var thtmp=(" + jsc + ")%(2*pi);if(thtmp>7*pi/4){ctx.line(coordinate.X2+2,coordinate.Y2-2);}if(thtmp>5*pi/4){ctx.line(coordinate.X1-2,coordinate.Y2-2);}if(thtmp>pi/4){ctx.line(coordinate.X1-2,coordinate.Y1+2);ctx.line(coordinate.X2+2,coordinate.Y1+2);}ctx.line(coordinate.X2+2,0);ctx.line(0,0);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            }
                            builder += "ctx.stroke();";
                        }
                ...
                if (tfuncs.length) {
                    for (var i = 0; i < tfuncs.length; i++) {
                        ...
                        if (0 && singularities.length) {
                             ...
                             */
                            builder += "var tinc=1/max(coordinate.width,coordinate.height);for(var theta=0;theta<(2*pi);theta+=tinc){var rtmp=(" + jsc + ");ctx.line(rtmp*cos(theta),rtmp*sin(theta));}ctx.line(" + jsc + ",0);";

                            if (fn.type == eqtype.greaterthan) {
                                builder += "ctx.line(coordinate.X2+2,0);ctx.line(coordinate.X2+2,coordinate.Y2-2);ctx.line(coordinate.X1-2,coordinate.Y2-2);ctx.line(coordinate.X1-2,coordinate.Y1+2);ctx.line(coordinate.X2+2,coordinate.Y1+2);ctx.line(coordinate.X2+2,0);";
                                builder += "ctx.globalAlpha=0.2;ctx.fill();ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                ...
                ret.plot = new Function("ctx, coordinate", builder);
