...
define(['jquery', 'naur', 'naur.math', 'naur.graphics.js', 'naur.graphics.math', '.'], function ($, NAUR) {
    NAUR.Graphics.Equation = (function () {
        var equation = {
            random:{
                index:0,
            ...
            Graph:function (n, disabled, color) {
                ...
                var t = {};
                var c = NAUR.Graphics.Math.compile(latex);
            ...
            gridlines:function (options) {
                ...
                for (i = 0.000000000001, c = 0; coordinate.range.X / i > this.config.gridlines.maxgridlines.X - 1; c++) {
                ...
                for (i = 0.000000000001, c = 0; coordinate.range.Y / i > this.config.gridlines.maxgridlines.Y - 1; c++) {
                    if (c % 3 == 1) i *= 2.5;
                    else i *= 2;
                }
                opt.ygridscale = i;
                ...
                opt.currx = NAUR.Math.arbFloor(coordinate.X1, opt.xgridscale);	//set it to before the lowest x-value on the screen
                opt.curry = NAUR.Math.arbFloor(coordinate.Y1, opt.ygridscale);
                opt.ymainaxis = -1;
                ...
                if (coordinate.Y2 >= 0 && coordinate.Y1 <= 0)    //y=0 appears on the screen - move the text to follow
                    opt.xmainaxis = layout.height - ((0 - coordinate.Y1) / (coordinate.Y2 - coordinate.Y1)) * layout.height + (this.config.gridlines.charHeight * 1.2);
                ...
                if (opt.xmainaxis > layout.height - (this.config.gridlines.charHeight / 2))
                    opt.xmainaxis = layout.height - 1;
                ...
                //Draw the axis
                if (opt.xaxis)
                    coordinate.ctx.fillRect(opt.xaxis - 0.5, 0, 1, layout.height);
                if (opt.yaxis)
                    coordinate.ctx.fillRect(0, opt.yaxis - 0.5, layout.width, 1);
            },
...
//                    if (jsgui.gridlines == "normal" || (jsgui.gridlines == "less" && Calc.roundFloat(currx) % Calc.roundFloat((this.xgridscale * 2)) == 0)) {
                    ...
                    coordinate.ctx.fillRect(xpos - 0.5, 0, 1, layout.height);
//                    }
...
//                    if (jsgui.gridlines == "normal" || (jsgui.gridlines == "less" && Calc.roundFloat(curry) % (Calc.roundFloat(this.ygridscale * 2)) == 0)) {
                    coordinate.ctx.fillStyle = "rgb(190,190,190)";
                    coordinate.ctx.fillRect(0, ypos - 0.5, layout.width, 1);
//                    }
                    ...
                    if (opt.curry != 0) {
                        ...
                        if (ypos + (this.config.gridlines.charHeight / 2) > layout.height) //cannot overflow the screen
                            ypos = layout.height - (this.config.gridlines.charHeight / 2) - 1;
                        if (ypos - 4 < 0)
                            ypos = 4;
