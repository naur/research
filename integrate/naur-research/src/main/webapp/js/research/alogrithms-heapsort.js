...
function BuildMaxHeap(A) {
    ...
    for (i = Math.floor(heapsize / 2); i > 0; i--) {
        NAUR.Message.showLine({message:'BuildMaxHeapify: ' + i});
        MaxHeapify(A, i);
    }
    NAUR.Message.showLine({message:A.toString()});
}
...
function MaxHeapify(A, i) {
    ...
    if (largest != i) {
        NAUR.Message.showLine({message:A.toString() + " -- " + 'i: ' + i + "[" + A[i] + "]" + ",    " + "largest: " + largest + "[" + A[largest] + "]"});
...
$(function () {
    ...
    $('input[type="button"]').on('click', function () {
        ...
        NAUR.Message.empty();
        NAUR.Message.prompt(A.toString());
        BuildMaxHeap(A);
//        $('#pre1').append('  a\r');
