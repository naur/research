            ...
            compile:function (n) {
                ...
                if (funcs.length) {
                    for (var i = 0; i < funcs.length; i++) {
                        ...
                        builder += "coordinate.ctx.beginPath();var x=coordinate.X1;coordinate.ctx.move(x," + jsc + ");";
                        if (0 && singularities.length) {
                            ...
                            builder += "for(var x=coordinate.X1;x<coordinate.X2;x+=(coordinate.X2-coordinate.X1)/coordinate.width){" + "coordinate.ctx.line(x," + jsc + ");}coordinate.ctx.stroke();";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "coordinate.ctx.line(coordinate.X2+2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X1-2,coordinate.Y1+2);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "coordinate.ctx.line(coordinate.X2+2,coordinate.Y2-2);coordinate.ctx.line(coordinate.X1-2,coordinate.Y2-2);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            }

                            builder += "coordinate.ctx.stroke();";
                        }
                ...
                if (yfuncs.length) {
                    for (var i = 0; i < yfuncs.length; i++) {
                        ...
                        //begin the path. Colour (fill and stroke) is already done for us in ui.js
                        builder += "coordinate.ctx.beginPath();var y=coordinate.Y2;coordinate.ctx.move(" + jsc + ",y);";
                        if (0 && singularities.length) {
                            //Better plot logic.
                            //For example, this would allow us to plot y=sqrt(1-x^2) from [-1,1] exactly with no abrupt endings.
                            ...
                            //The loop is within the function because calling a function is too slow in javascript (last time I checked)
                            //Plot each point: (jsc is the expression generated by CAS)
                            builder += "for(var y=coordinate.Y2;y<coordinate.Y1;y+=(coordinate.Y1-coordinate.Y2)/coordinate.height){" + "coordinate.ctx.line(" + jsc + ",y);}";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "coordinate.ctx.line(coordinate.X2+2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X2+2,coordinate.Y2-2);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "coordinate.ctx.line(coordinate.X1-2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X1-2,coordinate.Y2-2);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            }
                            builder += "coordinate.ctx.stroke();";
                ...
                if (rfuncs.length) {
                    for (var i = 0; i < rfuncs.length; i++) {
                        ...
                        //begin the path. Colour (fill and stroke) is already done for us in ui.js
                        builder += "coordinate.ctx.beginPath();var r=0;coordinate.ctx.move(0,0);";
                        if (0 && singularities.length) {
                            //Better plot logic.
                            //For example, this would allow us to plot y=sqrt(1-x^2) from [-1,1] exactly with no abrupt endings.
                            ...
                            //Plot each point: (jsc is the expression generated by CAS)
                            /*
                             We want to plot all points (x,y) ∈ ([coordinate.X1,coordinate.X2],[coordinate.Y2,coordinate.Y1])
                             r*(sin(x),cos(y)) ∈ (r*cos[coordinate.X1,coordinate.X2],r*sin[coordinate.Y2,coordinate.Y1])
                             ...
                             asin(y/r)=f(atan(y/x))
                             */
                            builder += "var rinc=(rmax-rmin)/coordinate.width;for(var r=rmin;r<rmax;r+=rinc){var thtmp=(" + jsc + ");coordinate.ctx.line(r*cos(thtmp),r*sin(thtmp));}";
                            if (fn.type == eqtype.greaterthan) {
                                builder += "coordinate.ctx.line(coordinate.X2+2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X2+2,coordinate.Y2-2);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "var thtmp=(" + jsc + ")%(2*pi);if(thtmp>7*pi/4){coordinate.ctx.line(coordinate.X2+2,coordinate.Y2-2);}if(thtmp>5*pi/4){coordinate.ctx.line(coordinate.X1-2,coordinate.Y2-2);}if(thtmp>pi/4){coordinate.ctx.line(coordinate.X1-2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X2+2,coordinate.Y1+2);}coordinate.ctx.line(coordinate.X2+2,0);coordinate.ctx.line(0,0);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            }
                            builder += "coordinate.ctx.stroke();";
                        }
                ...
                if (tfuncs.length) {
                    for (var i = 0; i < tfuncs.length; i++) {
                        ...
                        //begin the path. Colour (fill and stroke) is already done for us in ui.js
                        builder += "coordinate.ctx.beginPath();var theta=0;coordinate.ctx.move(" + jsc + "*cos(theta)," + jsc + "*sin(theta));";
                        if (0 && singularities.length) {
                            //Better plot logic.
                            //For example, this would allow us to plot y=sqrt(1-x^2) from [-1,1] exactly with no abrupt endings.
                             ...
                             */
                            builder += "var tinc=1/max(coordinate.width,coordinate.height);for(var theta=0;theta<(2*pi);theta+=tinc){var rtmp=(" + jsc + ");coordinate.ctx.line(rtmp*cos(theta),rtmp*sin(theta));}coordinate.ctx.line(" + jsc + ",0);";

                            if (fn.type == eqtype.greaterthan) {
                                builder += "coordinate.ctx.line(coordinate.X2+2,0);coordinate.ctx.line(coordinate.X2+2,coordinate.Y2-2);coordinate.ctx.line(coordinate.X1-2,coordinate.Y2-2);coordinate.ctx.line(coordinate.X1-2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X2+2,coordinate.Y1+2);coordinate.ctx.line(coordinate.X2+2,0);";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            } else if (fn.type == eqtype.lessthan) {
                                builder += "var theta=0;coordinate.ctx.line(" + jsc + "*cos(theta)," + jsc + "*sin(theta));";
                                builder += "coordinate.ctx.globalAlpha=0.2;coordinate.ctx.fill();coordinate.ctx.globalAlpha=1.0;";
                            }
                            builder += "coordinate.ctx.stroke()";
                        }
