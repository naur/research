...
define(['jquery', 'naur'], function($, NAUR) {
    ...
    NAUR.Location.DefaultGeoOptions = {
    ...
    if (navigator.geolocation) {
        NAUR.Location.enableGeoLocation = true;
    } else {
        NAUR.Location.enableGeoLocation = false;
    }

    NAUR.Location.Start = function (options) {
        var opt = $.extend({
            geoOptions:NAUR.Location.DefaultGeoOptions,
            success:null,
            error:null
        }, options);

        if (!NAUR.Location.enableGeoLocation) {
            if (opt.error)
                opt.error({code:NAUR.Location.ErrorCode.NOSUPPORT, message:"NAUR.Location.enableGeoLocation = false"});
            return
        }
    ...
    NAUR.Location.End = function (watchId) {
        if (!NAUR.Location.enableGeoLocation) {
            if (opt.error)
                opt.error({code:NAUR.Location.ErrorCode.NOSUPPORT, message:"NAUR.Location.enableGeoLocation = false"});
            return
    ...
    NAUR.Location.Current = function (options) {
        var opt = $.extend({
            geoOptions:NAUR.Location.DefaultGeoOptions,
            success:null,
            error:null
